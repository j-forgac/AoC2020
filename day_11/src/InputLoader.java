public class InputLoader {
	...
	public static String[][] getInput() {
		String[][] processedInput = new String[input.length + 2][input[0].length() + 2];
		for(int x = 0; x < processedInput.length;x++){
			for(int y = 0; y < processedInput[x].length; y++){
				if(x == 0 || x == processedInput.length - 1 || y == 0 || y == processedInput[x].length  - 1){
					processedInput[x][y] = "*";
				} else {
					processedInput[x][y] = Character.toString(input[x - 1].charAt(y - 1));
				}
			}
		}
